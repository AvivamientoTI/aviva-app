...
async function loginAs(page: import('@playwright/test').Page) {
    ...
    await page.getByLabel(/Contrase/i).fill(TEST_PASS);
    await page.getByRole('button', { name: /Iniciar Sesi/i }).click();
    await page.waitForURL(/^(?!.*login).*$/, { timeout: 10_000 });
}

test.describe('Planificador - sin sesion', () => {
    test('redirige a /login al acceder a /planning sin sesion', async ({ page }) => {
        await page.goto('/planning');
        await expect(page).toHaveURL(/login/, { timeout: 6000 });
    });
});

test.describe('Planificador - flujo basico', () => {
    test.skip(
